---
title: "Lab 1: Exercise - Answer Key "
author: " " 
format:
  pdf:
    documentclass: article
    geometry: top=2.25cm, bottom=2.25cm, left=1.91cm, right=1.91cm
    toc: false
    ...
    include-in-header:
      text: |
        ...
        \definecolor{codefg}{RGB}{155,89,182} 

        % fixed-height/ depth strut (tune these two numbers)
        % \rule[<depth>]{0pt}{<height>}
        \newcommand{\codestrut}{\rule[-0.15em]{0pt}{0.9em}}
...
header-includes:
  ...
  - \usepackage{setspace}
  - \AtBeginEnvironment{document}{\setstretch{1.25}}  # Apply to body
  - \usepackage{etoolbox}
  - \AtBeginEnvironment{longtable}{\singlespacing}   # Table row spacing normal
  - \AtBeginEnvironment{tabular}{\singlespacing}     # Also for tabular tables
...
unique(mpg$class[mpg$hwy_Lp100km_1 == max(mpg$hwy_Lp100km_1)])
```
...
quantile(mpg$hwy, c(0, 0.333, 0.666, 1)) # you can achieve the same using 
```
